/*
 * Nightcord â€” Local un-injector for Discord Desktop
...
function findAllDiscordResources() {
    ...
    // Filtrer uniquement les paths avec une injection Nightcord prÃ©sente
...
function uninject(resourcesDir) {
    ...
    // VÃ©rifier que le dossier app/ a bien Ã©tÃ© crÃ©Ã© par Nightcord
    if (existsSync(appDirPath)) {
        try {
            if (existsSync(join(appDirPath, "index.js"))) {
                ...
                if (!indexContent.includes("Nightcord Injector") && !indexContent.includes("Nightcord")) {
                    console.warn(`\x1b[33m[Nightcord] Le dossier app/ existe mais n'a pas l'air d'avoir Ã©tÃ© crÃ©Ã© par Nightcord.\x1b[0m`);
        ...
        console.log("[Nightcord] Suppression du dossier app/ injectÃ©...");
        rmSync(appDirPath, { recursive: true, force: true });
    } else {
        console.log("\x1b[33m[Nightcord] Aucun dossier app/ injectÃ© trouvÃ©.\x1b[0m");
    }

    // Restaurer le backup
    if (existsSync(backupPath) && !existsSync(appAsarPath)) {
        console.log("[Nightcord] Restauration _app.asar â†’ app.asar...");
        renameSync(backupPath, appAsarPath);
    } else if (existsSync(backupPath) && existsSync(appAsarPath)) {
        console.log("[Nightcord] app.asar dÃ©jÃ  prÃ©sent, nettoyage du backup...");
        rmSync(backupPath, { force: true });
    }

    console.log(`\x1b[32m[Nightcord] DÃ©sinjection rÃ©ussie depuis : ${resourcesDir}\x1b[0m`);
    console.log("\x1b[36m[Nightcord] RedÃ©marrez Discord pour appliquer les changements.\x1b[0m");
    return true;
}
...
if (allResources.length === 0) {
    console.error("\x1b[31m[Nightcord] Aucune installation Discord avec Nightcord injectÃ© trouvÃ©e.\x1b[0m");
    console.error("\x1b[33m           Assurez-vous que Nightcord a bien Ã©tÃ© injectÃ© via 'pnpm inject'.\x1b[0m");
    process.exit(1);
}

let uninjectCount = 0;
for (const res of allResources) {
    console.log(`\n[Nightcord] TrouvÃ© : ${res}`);
    if (uninject(res)) uninjectCount++;
}

if (uninjectCount === 0) {
    console.error("\x1b[31m[Nightcord] Aucune dÃ©sinjection rÃ©ussie.\x1b[0m");
    process.exit(1);
}

console.log(`\n\x1b[32m[Nightcord] ${uninjectCount}/${allResources.length} dÃ©sinjection(s) rÃ©ussie(s).\x1b[0m`);
