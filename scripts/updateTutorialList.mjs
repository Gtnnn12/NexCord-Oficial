...
async function main() {
    ...
    const res = await fetch("https://source.nightcord.st/api/v1/repos/nightcord/nightcord-tutorials/contents/videos");
    if (!res.ok) {
    ...
    const nightcordPluginsDir = path.resolve("src/nightcordplugins");

    const pluginDirs = [
        ...fs.readdirSync(pluginsDir).map(p => ({ dir: pluginsDir, name: p })),
        ...fs.readdirSync(nightcordPluginsDir).map(p => ({ dir: nightcordPluginsDir, name: p }))
    ];
