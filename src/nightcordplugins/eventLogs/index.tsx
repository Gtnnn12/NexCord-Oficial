...
function cacheMsg(msg: any) {
    ...
    // Support embeds (messages de bots)
...
function LogRow({ e, onClose }: { e: LogEntry; onClose?: () => void; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("NexCord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `NexCord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
