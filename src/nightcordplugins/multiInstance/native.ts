...
function saveSharedSettings(settings: Record<string, string>): void {
    try {
        ...
        console.warn("[NexCordMI] Failed to save shared settings:", e);
    }
}
...
function createTokenPreload(token: string, sharedSettings: Record<string, string> = {}): string {
    ...
    const innerLines = [
        ...
        "    } catch(e) {}",
        "  }",
        "  try { Object.defineProperty(window, '__NexCord_token', { value: RAW_TOKEN, writable: false, configurable: true }); } catch(e) {}",
        "  (function() {",
        "    var lastUI = 0;",
        ...
        "  })();",
        "  console.log('[NexCordMI] token preload active');",
        "})();"
    ].join("\n");
    ...
    const script = [
        ...
        "        .catch(function(err) { console.warn('[NexCordMI] mainWorld error:', err); });",
        "    }",
        "  } catch(e) { console.warn('[NexCordMI] preload error:', e); }",
        "})();"
    ].join("\n");
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const savedPartition = `persist:NexCord-mi-${userId}`;
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `NexCord [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[NexCordMI] setAppDetails failed:", err);
            }
        }
            ...
            wc.executeJavaScript(`
                (async () => {
                    ...
                    try {
                        ...
                        const ws = window.__NexCord_GW_WS__;
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[NexCordMI] arrangeSplit error:", e);
    }
}
