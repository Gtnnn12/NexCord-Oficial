...
import { NexCord_AVATAR_BASE64 } from "./avatarData";

export const NexCord_USER_ID = "999999999999999999";
export const NexCord_CHANNEL_ID = "999999999999999990";
export const NexCord_CREATED_AT = new Date("2024-01-01T00:00:00Z");
...
const LS_LAST_SEEN = "NexCord_official_dm_last_seen";
const LS_LAST_TS = "NexCord_official_dm_last_ts";
...
const API_URLS = [
    "https://social.NexCord.st/api/v1/timelines/public?local=true&limit=20",
    "https://social.NexCord.st/api/v1/timelines/public?limit=20"
];
...
const NexCordAvatarUrl = NexCord_AVATAR_BASE64;

const createChannelRecordFromServer = findByCodeLazy(".GUILD_TEXT]", "fromServer)");
const createMessageRecord = findByCodeLazy(".createFromServer(", ".isBlockedForMessage", "messageReference:");

function replaceDiscordWithNexCord(text: string): string {
    if (!text || typeof text !== "string") return text;
    return text
        .replace(/Official Discord Message/gi, "Official NexCord Message")
        ...
        .replace(/Discord ne te demandera/gi, "NexCord ne te demandera")
        .replace(/Discord will never ask/gi, "NexCord will never ask")
        .replace(/Official Discord/gi, "Official NexCord")
        .replace(/Discord Official/gi, "NexCord Official");
}

function isNexCordContext(node: Node): boolean {
    try {
        if (SelectedChannelStore?.getChannelId?.() === NexCord_CHANNEL_ID) return true;
        ...
        while (curr) {
            if (curr.getAttribute) {
                ...
                if (href && (href.includes(NexCord_CHANNEL_ID) || href.includes(NexCord_USER_ID))) return true;
                const ariaLabel = curr.getAttribute("aria-label");
                if (ariaLabel && ariaLabel.toLowerCase().includes("NexCord")) return true;
                const dataId = curr.getAttribute("data-list-item-id") || curr.getAttribute("data-channel-id");
                if (dataId && (dataId.includes(NexCord_CHANNEL_ID) || dataId.includes(NexCord_USER_ID))) return true;
            }

            if (curr.querySelector) {
                const hasNexCord = curr.querySelector(`[href*="${NexCord_CHANNEL_ID}"], [href*="${NexCord_USER_ID}"], [data-list-item-id*="${NexCord_CHANNEL_ID}"]`);
                if (hasNexCord) return true;
            }

            if (curr.textContent && curr.textContent.includes("NexCord") && (curr.classList?.contains?.("channel") || curr.getAttribute?.("role") === "listitem" || curr.classList?.contains?.("interactive") || curr.classList?.contains?.("layout"))) {
                return true;
            }
...
let hoveredIsNexCord = false;

function scanAndFixNexCordDmElements() {
    if (isUpdatingDom) return;
    isUpdatingDom = true;
    try {
        // 1. Target specifically the NexCord DM list item
        const NexCordDmElements = document.querySelectorAll(
            `[href*="${NexCord_CHANNEL_ID}"], [href*="${NexCord_USER_ID}"], [data-list-item-id*="${NexCord_CHANNEL_ID}"]`
        );

        for (const el of Array.from(NexCordDmElements)) {
            // Fix subtext inside this specific NexCord DM element
            ...
            for (const sub of Array.from(subtexts)) {
                if (sub.textContent && /Official Discord Message|Message officiel de Discord|Message de discorde officiel/i.test(sub.textContent)) {
                    sub.textContent = replaceDiscordWithNexCord(sub.textContent);
                }
            }
            ...
            if (aria && /Discord/i.test(aria)) {
                el.setAttribute("aria-label", replaceDiscordWithNexCord(aria));
            }

            // Track hover for tooltip
            if (!(el as any).__ncHoverBound) {
                (el as any).__ncHoverBound = true;
                el.addEventListener("mouseenter", () => { hoveredIsNexCord = true; });
                el.addEventListener("mouseleave", () => { hoveredIsNexCord = false; });
            }
        }

        // 2. If tooltip is visible and hovered element is NexCord, fix tooltip text
        if (hoveredIsNexCord) {
            ...
            for (const tip of Array.from(tooltips)) {
                if (tip.textContent && /Official Discord Message|Message officiel de Discord|Message de discorde officiel/i.test(tip.textContent)) {
                    tip.textContent = replaceDiscordWithNexCord(tip.textContent);
                }
            }
        }
    } catch {} finally {
...
function handleGlobalClick(e: MouseEvent) {
    if (SelectedChannelStore.getChannelId() !== NexCord_CHANNEL_ID) return;
    ...
    if (btn) {
        ...
        if (txt === "learn more" || txt === "en savoir plus") {
            e.preventDefault();
            e.stopPropagation();
            window.open("https://NexCord.st", "_blank");
        }
    }
}

const rawNexCordUser = {
    id: NexCord_USER_ID,
    username: "NexCord",
    discriminator: "0",
    globalName: "NexCord",
    global_name: "NexCord",
    avatar: "NexCord_avatar",
    avatarURL: NexCordAvatarUrl,
    ...
    getAvatarURL: () => NexCordAvatarUrl,
    getAvatarSource: () => ({ uri: NexCordAvatarUrl }),
    getAvatarDecorationURL: () => null,
    getBannerURL: () => null,
    toString: () => `<@${NexCord_USER_ID}>`,
};

const rawNexCordUserProxy = new Proxy(rawNexCordUser, {
...
export const NexCord_USER = proxyLazy(() => {
    try {
        ...
        if (URec) {
            ...
            u.getAvatarURL = () => NexCordAvatarUrl;
            u.getAvatarSource = () => ({ uri: NexCordAvatarUrl });
    ...
    return rawNexCordUserProxy;
});

const rawNexCordChannel = {
    id: NexCord_CHANNEL_ID,
    type: 1, // DM
    name: "NexCord",
    guild_id: null,
    guildId: null,
    getGuildId: () => null,
    getRecipientId: () => NexCord_USER_ID,
    recipients: [NexCord_USER_ID],
    rawRecipients: [NexCord_USER],
    isSystem: () => true,
    isOfficialSystem: () => true,
    isSystemDM: () => true,
    isDM: () => true,
...
const rawNexCordChannelProxy = new Proxy(rawNexCordChannel, {
    get(target, prop, receiver) {
        if (prop in target) return Reflect.get(target, prop, receiver);
        if (typeof prop === "string") {
            if (prop.startsWith("is") || prop.startsWith("has") || prop.startsWith("can")) {
...
export const NexCord_CHANNEL = proxyLazy(() => {
    try {
        if (typeof createChannelRecordFromServer === "function") {
            ...
            if (ch) {
                ...
                ch.isSystemDM = () => true;
                ch.getRecipientId = () => NexCord_USER_ID;
                return ch;
            }
        }
    } catch {}
    return rawNexCordChannelProxy;
});
...
const STREAMING_URLS = [
    "wss://social.NexCord.st/api/v1/streaming?stream=public:local",
    "wss://social.NexCord.st/api/v1/streaming?stream=public",
    "wss://social.NexCord.st/api/v1/streaming/public/local",
    "wss://social.NexCord.st/api/v1/streaming/public"
];
...
function injectSinglePost(post: any) {
    ...
    if (SelectedChannelStore.getChannelId() !== NexCord_CHANNEL_ID) {
        unreadCount++;
    ...
    try {
        ...
        ws.onmessage = (event) => {
            try {
                ...
                console.warn("[NexCordOfficialDM] Error parsing streaming message:", err);
            }
        };
...
export function UrgentMessageModal({ onClose, transitionState }: UrgentMessageModalProps) {
    const handleConfirm = () => {
        onClose();
        try {
            NavigationRouter.transitionTo(`/channels/@me/${NexCord_CHANNEL_ID}`);
        } catch {}
    };

    return (
        <ModalRoot transitionState={transitionState} size={ModalSize.DYNAMIC} className="NexCord-urgent-modal-root">
            <div data-mana-component="modal" className="NexCord-urgent-container container__8a031 size-md__8a031 padding-size-lg__8a031" style={{ opacity: 1, transform: "scale(1)" }}>
                <div className="NexCord-urgent-header-gradient container_a62383 blue_a62383 headerGradient__8a031" style={{ "--custom-gradient-offset-bottom": "0%" } as any}>
                    <header className="section__8a031 header__8a031 headerCentered__8a031">
                        <div data-align="stretch" data-justify="start" data-direction="vertical" data-wrap="false" data-full-width="true" className="stack_dbd263" style={{ gap: "var(--space-8)", padding: "var(--space-0)" }}>
                            <div className="NexCord-urgent-header-top headerLayout__8a031">
                                ...
                                <div className="NexCord-urgent-graphic-wrap headerMain__8a031">
                                    <div className="headerGraphic__8a031">
                                        <div className="headerGraphicContainer__8a031">
                                            <div className="container__8ef77 aspect-ratio-16/9__8ef77">
                                                <img className="NexCord-urgent-img image__8ef77" alt="" draggable={false} src="https://cdn.discordapp.com/assets/content/88cde2b0ab4c8015cee8fdb8732b85b01df4fc78a75b3aa5e621539ea94b1803.svg" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="headerTrailing__8a031 headerTrailingAbsolute__8a031">
                                    <button data-mana-component="button" role="button" className="NexCord-urgent-close-btn button_a22cb0 md_a22cb0 color-mix_a22cb0" type="button" aria-label="Close" onClick={onClose}>
                            ...
                            <h1 className="NexCord-urgent-title heading-xl/semibold_cf4812 defaultColor__5345c headerTitle__8a031" id="heading-_r_53_" data-text-variant="heading-xl/semibold" style={{ color: "var(--text-strong, #ffffff)" }}>
                                {t("Urgent Message")}
                            </h1>
                            <div className="headerSubtitleWrapper__8a031">
                                <div className="NexCord-urgent-subtitle text-md/normal_cf4812 headerSubtitle__8a031" data-text-variant="text-md/normal" style={{ color: "var(--text-subtle, #dbdee1)" }}>
                                    {t("There's an official message from the NexCord team that needs your attention.")}
                                </div>
                            </div>
                        </div>
                    </header>
                </div>
                <footer className="NexCord-urgent-footer actionBar__8a031 section__8a031">
                    <div className="actionBarTrailing__8a031 actionBarTrailingFullWidth__8a031">
                        <div data-align="stretch" data-justify="start" data-direction="horizontal" data-wrap="true" data-full-width="true" className="stack_dbd263" style={{ gap: "var(--space-8)", padding: "var(--space-0)" }}>
                            <button
                                ...
                                role="button"
                                className="NexCord-urgent-ok-btn button_a22cb0 md_a22cb0 primary_a22cb0 hasText_a22cb0 fullWidth_a22cb0"
                                type="button"
...
async function checkAndInjectPosts(isInitialLoad = false) {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            if (newPostCount > 0) {
                if (!settings.store.isDmClosed && SelectedChannelStore.getChannelId() !== NexCord_CHANNEL_ID) {
        ...
        console.warn("[NexCordOfficialDM] Error checking posts:", e);
    }
}
...
const NexCordProfileSections = ErrorBoundary.wrap(({ isSideBar }: { isSideBar: boolean }) => (
    <>
        <Section
        ...
        >
            <BaseText size="sm" color="text-normal">
                {formatDate(NexCord_CREATED_AT)}
            </BaseText>
        </Section>
    </>
), { noop: true });

export function NexCordSystemBanner() {
    return (
        <div className="NexCord-official-banner">
            <div className="NexCord-official-banner-content">
            ...
            <button
                className="NexCord-official-banner-btn"
                onClick={() => window.open("https://NexCord.st", "_blank")}
            >
                En savoir plus
            </button>
        </div>
...
export default definePlugin({
    name: "NexCordOfficialDM",
    description: "Official NexCord System DM channel with verified badge and Mastodon feed integration",
    ...
    patches: [
        {
            ...
            replacement: {
                ...
                replace: "(Vencord.Api.ChatButtons._injectButtons($1,arguments[0]),$self.isNexCordChannel(arguments[0])?$self.renderBanner():$&)"
            }
        },
        // Inject username + creation date in DM sidebar profile panel
        {
            ...
            replacement: {
                ...
                replace: "$1,$self.renderNexCordProfileSections({userId:$2,isSideBar:true})"
            }
        },
        ...
        // Direct React Component patch for DM list â€” filters out NexCord DM when closed
        {
            ...
            replacement: {
                ...
                replace: "privateChannelIds:$self.filterNexCordDm($1)"
            }
        }
    ],

    filterNexCordDm(channelIds: any) {
        if (!Array.isArray(channelIds)) return channelIds;
        const closed = settings.store.isDmClosed;
        if (closed) {
            return channelIds.filter((id: string) => id !== NexCord_CHANNEL_ID);
        }
        return channelIds;
    },

    renderNexCordProfileSections({ userId, isSideBar }: { userId: string, isSideBar: boolean }) {
        if (userId !== NexCord_USER_ID) return null;
        return <NexCordProfileSections isSideBar={isSideBar} />;
    },

    isNexCordChannel(props: any) {
        const id = props?.channel?.id || props?.id;
        if (id === NexCord_CHANNEL_ID) return true;
        if (props?.channel?.name === "NexCord" || props?.name === "NexCord") return true;
        if (props?.channel?.recipients?.includes?.(NexCord_USER_ID)) return true;
        if (SelectedChannelStore.getChannelId() === NexCord_CHANNEL_ID) return true;
        return false;
    },

    getNexCordSystemText() {
        return "Official NexCord Message";
    },

    renderBanner() {
        return <NexCordSystemBanner />;
    },
    ...
    start() {
        ...
        try {
            ...
            fd.dispatch = function(action: any, ...args: any[]) {
                if (
                    ...
                    (action?.channel?.id === NexCord_CHANNEL_ID || action?.channelId === NexCord_CHANNEL_ID) &&
            ...
            setTimeout(() => {
                try {
                    FluxDispatcher.dispatch({
                        type: "CHANNEL_DELETE",
                        channelId: NexCord_CHANNEL_ID,
                        channel: { id: NexCord_CHANNEL_ID, type: 1 },
                    });
                } catch {}
                try { (findStore("ChannelStore") as any)?.emitChange(); } catch {}
                try { (findStore("PrivateChannelSortStore") as any)?.emitChange(); } catch {}
        ...
        waitFor(["fromTimestamp", "extractTimestamp"], (m: any) => {
            if (m && typeof m.extractTimestamp === "function" && !unpatchSnowflakeUtils) {
                ...
                m.extractTimestamp = function(snowflake: string) {
                    if (snowflake === NexCord_USER_ID || snowflake === "999999999999999999") {
        ...
        waitFor(["useName", "getGlobalName"], (m: any) => {
            if (m && typeof m.formatForDisplay === "function") {
                ...
                m.formatForDisplay = function(user: any, ...args: any[]) {
                    ...
                    if (id === NexCord_USER_ID || id === "999999999999999999") return "NexCord";
        ...
        try {
            if (DisplayProfileUtils?.getDisplayProfile) {
                ...
                DisplayProfileUtils.getDisplayProfile = function(userId: string, ...args: any[]) {
                    if (userId === NexCord_USER_ID || userId === "999999999999999999") {
            ...
            if (IconUtils) {
                if (typeof IconUtils.getUserAvatarURL === "function") {
                    ...
                    IconUtils.getUserAvatarURL = function(user: any, ...args: any[]) {
                        ...
                        if (id === NexCord_USER_ID || id === "999999999999999999") return NexCordAvatarUrl;
            ...
            if (URec?.prototype) {
                if (typeof URec.prototype.getAvatarURL === "function") {
                    ...
                    URec.prototype.getAvatarURL = function(this: any, ...args: any[]) {
                        if (this?.id === NexCord_USER_ID || this?.id === "999999999999999999") return NexCordAvatarUrl;
                ...
                if (typeof URec.prototype.getAvatarSource === "function") {
                    ...
                    URec.prototype.getAvatarSource = function(this: any, ...args: any[]) {
                        if (this?.id === NexCord_USER_ID || this?.id === "999999999999999999") return { uri: NexCordAvatarUrl };
        ...
        // Patch UserProfileStore to return Staff badge, blue banner and NexCord profile
        try {
            ...
            if (UserProfileStore) {
                ...
                if (typeof UserProfileStore.isFetchingProfile === "function") {
                    ...
                    UserProfileStore.isFetchingProfile = function(userId: string) {
                        if (userId === NexCord_USER_ID) return false;
                ...
                if (typeof UserProfileStore.getUserProfileFetchStatus === "function") {
                    ...
                    UserProfileStore.getUserProfileFetchStatus = function(userId: string) {
                        if (userId === NexCord_USER_ID) return "SUCCESS";
        ...
        // Patch i18n intl methods for NexCord official strings (only when in NexCord context)
        try {
            if (i18n?.intl?.string) {
                ...
                i18n.intl.string = function(descriptor: any) {
                    try {
                        ...
                        if (typeof res === "string" && SelectedChannelStore?.getChannelId?.() === NexCord_CHANNEL_ID) {
                            return replaceDiscordWithNexCord(res);
                        }
                        return res;
                    } catch {
                    ...
                    try {
                        ...
                        if (SelectedChannelStore?.getChannelId?.() === NexCord_CHANNEL_ID) {
                            ...
                            if (Array.isArray(res)) {
                                return res.map(item => typeof item === "string" ? replaceDiscordWithNexCord(item) : item);
                            }
                        }
                        return res;
                    } catch {
        ...
        // Patch ReadStateStore to show red ping badge for new unread NexCord posts
        try {
            ...
            if (ReadStateStore) {
                ...
                ReadStateStore.getMentionCount = function(channelId: string) {
                    if (channelId === NexCord_CHANNEL_ID) return unreadCount;
                ...
                ReadStateStore.hasUnread = function(channelId: string) {
                    if (channelId === NexCord_CHANNEL_ID) return unreadCount > 0;
        ...
        try {
            ...
            if (MessageActions?.fetchMessages) {
                ...
                MessageActions.fetchMessages = function(opts: any) {
                    if (opts?.channelId === NexCord_CHANNEL_ID) {
                        checkAndInjectPosts(true);
                        return Promise.resolve();
                    }
            ...
            if (HTTP) {
                if (HTTP.get) {
                    ...
                    HTTP.get = function(opts: any) {
                        ...
                        if (url && (url.includes(NexCord_CHANNEL_ID) || url.includes(NexCord_USER_ID) || url.includes("999999999999999999"))) {
        ...
        UserStore.getUser = function(id: string) {
            if (id === NexCord_USER_ID || id === "999999999999999999") return NexCord_USER;
        ...
        if ((UserStore as any).getUsers) {
            ...
            (UserStore as any).getUsers = function() {
                ...
                if (!users[NexCord_USER_ID]) {
                    return { ...users, [NexCord_USER_ID]: NexCord_USER };
                }
                return users;
            };
        ...
        try {
            ...
            actionMods.forEach((mod: any) => {
                ...
                closeMethods.forEach(methodName => {
                    if (typeof mod[methodName] === "function" && !mod[methodName].__NexCordPatched) {
                        ...
                        mod[methodName] = function(...args: any[]) {
                            ...
                            if (targetId === NexCord_CHANNEL_ID || targetId === "999999999999999990") {
                                settings.store.isDmClosed = true;
                                                                unreadCount = 0;
                                try {
                                    FluxDispatcher.dispatch({
                        ...
                        mod[methodName].__NexCordPatched = true;
                    }
                });

                if (typeof mod.openPrivateChannel === "function" && !mod.openPrivateChannel.__NexCordPatched) {
                    ...
                    mod.openPrivateChannel = function(...args: any[]) {
                        ...
                        if (targetId === NexCord_USER_ID || targetId === "999999999999999999") {
                            settings.store.isDmClosed = false;
                                                        unreadCount = 0;
                            setTimeout(() => {
                                try {
                            ...
                            return Promise.resolve(NexCord_CHANNEL_ID);
                        }
                        return origOpen.apply(this, args);
                    };
                    mod.openPrivateChannel.__NexCordPatched = true;
                }
            });
        } catch {}
        ...
        ChannelStore.getChannel = function(id: string) {
            if (id === NexCord_CHANNEL_ID) {
                // Always return the channel so navigation works (even when closed)
                // Visibility in the DM list sidebar is handled by filterNexCordDm
                return NexCord_CHANNEL;
            }
        ...
        if (channelStoreAny.getMutablePrivateChannels) {
            ...
            channelStoreAny.getMutablePrivateChannels = function() {
                ...
                const closed = settings.store.isDmClosed;
                if (!closed && !chs[NexCord_CHANNEL_ID]) {
                    return { ...chs, [NexCord_CHANNEL_ID]: NexCord_CHANNEL };
                }
                if (closed && chs[NexCord_CHANNEL_ID]) {
                    const copy = { ...chs };
                    delete copy[NexCord_CHANNEL_ID];
                    return copy;
                }
                return chs;
            };
...
function getChannelTimestamp(channelId: string): number {
    ...
    return 0;
        ...
        const patchPrivateChannelSortStore = (SortStore: any) => {
            if (!SortStore || (SortStore as any).__NexCordPatched) return;
            (SortStore as any).__NexCordPatched = true;
            ...
            const patchSortFn = (fnName: string) => {
                if (typeof SortStore[fnName] === "function") {
                    ...
                    const entryToInsert = isObjectArray ? NexCord_CHANNEL : NexCord_CHANNEL_ID;
                    ...
                    SortStore[fnName] = function(...args: any[]) {
                        ...
                        try {
                            let NexCordTs = lastPostTimestamp;
                            const ChannelStore = findStore("ChannelStore") as any;
                            const ncCh = ChannelStore?.getChannel?.(NexCord_CHANNEL_ID);
                            if (ncCh?.lastMessageId) {
                                const ncExtracted = SnowflakeUtils.extractTimestamp(ncCh.lastMessageId);
                                if (ncExtracted > NexCordTs) NexCordTs = ncExtracted;
                            }
                    ...
                    sortUnpatches.push(() => {
                        ...
                        delete (SortStore as any).__NexCordPatched;
                    });
                }
            };
        ...
        // Patch MessageRequestStore to explicitly exclude NexCord_CHANNEL_ID from spam / message requests
        try {
            ...
            if (MsgReqStore) {
                if (typeof MsgReqStore.isSpam === "function") {
                    ...
                    MsgReqStore.isSpam = function(channelId: string) {
                        if (channelId === NexCord_CHANNEL_ID) return false;
