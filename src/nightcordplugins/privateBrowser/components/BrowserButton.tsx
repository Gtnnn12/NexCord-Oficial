...
const TABS_STORAGE_KEY = "NexCord_privatebrowser_tabs";
const ACTIVE_TAB_STORAGE_KEY = "NexCord_privatebrowser_active_tab";
...
export function BrowserButton() {
    ...
    React.useEffect(() => {
        ...
        // Without this, every iframe in Discord (NexCordNews, YouTube, etc.) would
...
function renderTabs() {
    ...
    // from non-browser iframes (NexCordNews, YouTube, etc.) are ignored.
    try {
        const s = new Set<string>();
        for (const t of tabs) {
            if (!t.url) continue;
