...
async function isExistingUser(): Promise<boolean> {
    try {
        if (await DataStore.get<boolean>("NexCord_installed")) return true;
...
export default definePlugin({
    ...
    async start() {
        ...
        if (rawPacked === null || rawPacked === undefined) {
            ...
            if (existingUser) {
                // Existing user updating NexCord: keep buttons unpacked
                await savePacked([]);
                await DataStore.set("NexCord_installed", true);
            } else {
