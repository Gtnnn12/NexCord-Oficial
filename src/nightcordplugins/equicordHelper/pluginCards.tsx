...
import { EQUIBOT_USER_ID, NexCord_BOT_USER_ID } from "@utils/constants";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of NexCord. Try updating!";
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    // Process components Ã¢â‚¬â€ NexCord Bot (domain, Component v2 Container format)
    if (message.author.id === NexCord_BOT_USER_ID) {
