...
export function CloudSyncModal({ initialMode = "sync", onClose, onCompleted }: CloudSyncModalProps) {
    ...
    const handleConnectCloud = async () => {
        ...
        try {
            ...
            if (ok) {
                ...
                setCloudToken(token);
                showToast(t("Connected to NexCord Cloud!"), Toasts.Type.SUCCESS);
            }
        } catch {
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            setMeta(updatedMeta);
            showToast(t("2FA Vault encrypted & synced to NexCord Cloud!"), Toasts.Type.SUCCESS);
            onCompleted?.();
            onClose();
        } catch (err: any) {
    ...
    const handleRestore = async () => {
        ...
        try {
            ...
            if (err?.message === "NO_VAULT_FOUND") {
                setError(t("No encrypted vault found in NexCord Cloud"));
            } else {
        ...
        <div className="totp-cloud-modal">
                ...
                <div className="totp-cloud-title-wrap">
                    ...
                    <div>
                        <Heading level={2} style={{ color: "#ffffff", margin: 0, fontSize: 18 }}>
                            {t("NexCord 2FA Cloud Vault")}
                        </Heading>
                        <Paragraph size="xs" color="text-muted">
                            {t("Zero-Knowledge End-to-End Encryption (cloud.NexCord.st)")}
                        </Paragraph>
                    </div>
                </div>
            ...
            <div className="totp-cloud-warning-card">
                ...
                <Paragraph size="xs" style={{ color: "var(--text-normal, #dbdee1)", marginTop: 8, lineHeight: 1.45 }}>
                    {t("Your 2FA secrets are encrypted client-side using cascaded AES-256-GCM and ChaCha20-Poly1305 before being sent to NexCord Cloud. Only you possess the key.")}
                </Paragraph>
                ...
                <div className="totp-cloud-danger-box">
                    ...
                    <Paragraph size="xs" style={{ color: "#f2f3f5", margin: 0, lineHeight: 1.4 }}>
                        {t("This Master Password is the ONLY key to decrypt your 2FA vault. NexCord servers NEVER store this password and CANNOT reset it. You must store this password in a safe place. If you lose it, your 2FA backup will be permanently lost.")}
                    </Paragraph>
                </div>
            </div>
