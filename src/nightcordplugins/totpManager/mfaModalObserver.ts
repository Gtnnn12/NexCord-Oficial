...
try {
    lastLoginEmail = localStorage.getItem("NexCord_last_login_email") || "";
} catch {}
...
function setLastLoginIdentifier(identifier: string) {
    ...
    if (clean.includes("@")) {
        ...
        try {
            localStorage.setItem("NexCord_last_login_email", clean);
        } catch {}
...
function fillCodeIntoInput(input: HTMLInputElement, code: string) {
    ...
    input.dataset.NexCordFilledCode = code;
...
function isInside2FASetupModal(el: HTMLElement): boolean {
    ...
    const hasSecret = !!modal.querySelector('[class*="secret__"], .secret__07d82, [class*="qrCode_"], img[alt*="qr" i], [class*="qrCodeOverlay_"], .NexCord-2fa-quickadd-btn');
...
function injectAutofillBanner(input: HTMLInputElement, accounts: TotpAccount[]) {
    ...
    if (wrapper?.nextElementSibling?.classList.contains("NexCord-2fa-autofill-card")) return;
    if (input.parentElement?.querySelector(".NexCord-2fa-autofill-card")) return;

    const card = document.createElement("div");
    card.className = "NexCord-2fa-autofill-card";
    ...
    function renderCardContent() {
        ...
        card.innerHTML = `
            <div class="NexCord-2fa-autofill-left">
                <div class="NexCord-2fa-autofill-info">
                    <div class="NexCord-2fa-autofill-title">
                        <span class="NexCord-2fa-acc-label">${displayName}</span>
                    </div>
                    <div class="NexCord-2fa-autofill-code-preview">
                        <span class="NexCord-2fa-live-otp">------</span>
                        <span class="NexCord-2fa-live-timer">30s</span>
                    </div>
                </div>
            </div>
            <div class="NexCord-2fa-autofill-right">
                ${sorted.length > 1 ? `
                    <select class="NexCord-2fa-acc-select" title="${t("Select Account")}">
        ...
        const otpSpan = card.querySelector<HTMLElement>(".NexCord-2fa-live-otp");
        const timerSpan = card.querySelector<HTMLElement>(".NexCord-2fa-live-timer");
        const selectEl = card.querySelector<HTMLSelectElement>(".NexCord-2fa-acc-select");
...
function scanForMfaCodeInputs() {
    ...
    document.querySelectorAll('[data-mana-component="modal"], [class*="modal__"], [class*="container__8a031"], [role="dialog"]').forEach(modal => {
        if (isInside2FASetupModal(modal as HTMLElement)) {
            modal.querySelectorAll(".NexCord-2fa-autofill-card").forEach(c => c.remove());
        }
    });
    ...
    for (const secretEl of Array.from(secretEls)) {
        ...
        if (!modal || secretEl.parentElement?.querySelector(".NexCord-2fa-quickadd-btn")) continue;

        const btn = document.createElement("button");
        btn.className = "NexCord-2fa-quickadd-btn";
        btn.type = "button";
        btn.innerText = t("Add to NexCord 2FA");
        btn.style.cssText = `
            display: flex;
            align-items: center;
        ...
        btn.onclick = (e) => {
            ...
            getTotpAccounts().then(async accounts => {
                ...
                try {
                    ...
                    showToast(t("Discord 2FA saved in NexCord & code filled!"), Toasts.Type.SUCCESS);
...
export function stopMfaModalObserver() {
    ...
    document.querySelectorAll(".NexCord-2fa-autofill-card, .NexCord-2fa-quickadd-btn").forEach(el => el.remove());
}
