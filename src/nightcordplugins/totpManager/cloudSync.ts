...
export const TOTP_VAULT_DATASTORE_KEY = "NexCord_totp_encrypted_vault";
export const TOTP_VAULT_METADATA_KEY = "NexCord_totp_vault_meta";
...
const API_BASES = ["https://api.NexCord.st", "https://cloud.NexCord.st"];
const PLUGIN_KEYS = ["totp-manager", "totpManager", "totp_manager"];

/**
 * Retrieves a valid OAuth2 session token for NexCord Cloud.
 */
export async function getValidCloudToken(promptAuth = false): Promise<string | null> {
    ...
    if (!token) {
        token = (await DataStore.get<string>("NexCord_oauth_token")) || undefined;
    }
    if (!token) {
...
export async function pullEncryptedVaultFromCloud(): Promise<EncryptedVaultEnvelope | null> {
    // 1. Try Direct PluginSync API (api.NexCord.st / cloud.NexCord.st)
    try {
        const token = await getValidCloudToken(false);
        if (token) {
            for (const base of API_BASES) {
...
/**
 * Pulls and decrypts accounts from NexCord Cloud using the Master Password and restores them into local store.
 */
