...
export type BadgeSource = "vencord" | "equicord" | "NexCord" | "globalbadges" | "illegalcord";
...
export async function loadOwnHiddenBadgeSources(userId: string) {
    ...
    try {
        const localData = localStorage.getItem("NexCord_hidden_badges");
        if (localData) {
    ...
    try {
        ...
        if (result?.config?.settings?.hidden !== undefined) {
            ...
            localStorage.setItem("NexCord_hidden_badges", JSON.stringify(hidden));
        }
    } catch (e) {
