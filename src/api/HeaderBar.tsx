    ...
    return (
        <Tooltip text={tooltip ?? ""} position={position} shouldShow={tooltip != null}>
            {({ onMouseEnter, onMouseLeave }) => (
                <Clickable
                    {...{ innerRef: ref } as any}
                    className={classes(HeaderBarClasses.clickable, "NexCord-header-btn", className)}
...
try { _stealthActive = localStorage.getItem("NexCord_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("NexCord_stealthMode", "1") : localStorage.removeItem("NexCord_stealthMode"); } catch { }
}

const NON_REACT_SELECTORS = [
    "#NexCord-titlebar-btn",
    "#NexCord-titlebar-link-style",
    ".nai-nav-item",
];
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("NexCord-stealth"); else document.body?.classList.remove("NexCord-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("NexCord-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("NexCord-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("NexCord-stealth-change"));
}
...
try { _compactActive = localStorage.getItem("NexCord_compactMode") === "1"; } catch { }
...
function persistCompact(v: boolean) {
    try { v ? localStorage.setItem("NexCord_compactMode", "1") : localStorage.removeItem("NexCord_compactMode"); } catch { }
}

export function syncCompactBodyClass() {
    try {
        const stored = localStorage.getItem("NexCord_compactMode");
    ...
    try {
        if (_compactActive) {
            document.body?.classList.add("NexCord-compact");
        } else {
            document.body?.classList.remove("NexCord-compact");
        }
    } catch { }
...
export function toggleCompactMode() {
    ...
    _notifyCompactChange();
    try { if (_compactActive) document.body?.classList.add("NexCord-compact"); else document.body?.classList.remove("NexCord-compact"); } catch { }
    return _compactActive;
}

if (_compactActive) {
    try { document.body?.classList.add("NexCord-compact"); } catch { }
}

export const compactListeners = new Set<() => void>();
export function _notifyCompactChange() {
    compactListeners.forEach(fn => fn());
    window.dispatchEvent(new Event("NexCord-compact-change"));
}
...
function CompactSettingsPopout({ closePopout }: { closePopout: () => void; }) {
    ...
    useEffect(() => {
        ...
        window.addEventListener("NexCord-compact-change", listener);
        window.addEventListener("NexCord-stealth-change", listener);
        return () => {
            compactListeners.delete(listener);
            stealthListeners.delete(listener);
            window.removeEventListener("NexCord-compact-change", listener);
            window.removeEventListener("NexCord-stealth-change", listener);
        };
    }, []);
        ...
        <div className="nc-settings-popout">
            ...
            <div className="nc-settings-popout-row" onClick={() => toggleStealthMode()}>
                <div className="nc-settings-popout-row-info">
                    ...
                    <div className="nc-settings-popout-row-desc">Hide all NexCord UI elements</div>
                </div>
...
export function _addHeaderBarButtons() {
    return [
        <style key="NexCord-headerbar-style">{`
            .NexCord-header-btn {
                display: flex;
                align-items: center;
                margin: 0 2px;
