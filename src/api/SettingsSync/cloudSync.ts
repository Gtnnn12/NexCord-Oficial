...
async function buildLocalData(): Promise<Map<string, Uint8Array>> {
    ...
    const totpVault = await DataStore.get("NexCord_totp_encrypted_vault");
    if (totpVault) data.set("dataStore/NexCord_totp_encrypted_vault", encoder.encode(JSON.stringify(totpVault)));

    const totpMeta = await DataStore.get("NexCord_totp_vault_meta");
    if (totpMeta) data.set("dataStore/NexCord_totp_vault_meta", encoder.encode(JSON.stringify(totpMeta)));
