...
handleSync(IpcEvents.GET_GIT_HASH, () => NexCord_GIT_HASH);
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.NexCordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { NexCordDir } = State.store;
    if (!NexCordDir) return;

    const stats = await stat(NexCordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(NexCordDir);
});
