...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} NexCord Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} NexCord's recommended Steam Deck controller settings?
