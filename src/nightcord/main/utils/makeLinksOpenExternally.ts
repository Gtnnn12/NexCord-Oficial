...
export function makeLinksOpenExternally(win: BrowserWindow) {
    win.webContents.setWindowOpenHandler(({ url, frameName, features }) => {
        console.log("[NexCord][LINK] setWindowOpenHandler url=", url, "frameName=", frameName);
        try {
        ...
        if (OVERLAY_FRAME_NAMES.has(frameName) || (frameName && frameName.includes("Overlay"))) {
            console.log("[NexCord] Blocked overlay popout (overlay unsupported):", frameName);
            return { action: "deny" };
        }
    ...
    win.webContents.on("did-create-window", (childWin, { frameName, options, url }: any) => {
        console.log("[NexCord][LINK] did-create-window url=", url, "frameName=", frameName);
