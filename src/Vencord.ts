 ...
 * NexCord-index.js will apply the staged files on the next restart.
 */
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A NexCord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair NexCord", err);
        }
    });
...
async function checkDiscordVersionChange() {
    if (IS_WEB || IS_UPDATER_DISABLED) return;

    try {
        const latest = getLatestDiscordApp();
        if (!latest) return;

        const expected = localStorage.getItem("NexCord_lastDiscordPath");
        const res = latest + "\\resources";
        const hasNexCord = fs.existsSync(res + "\\NexCord.asar") || fs.existsSync(res + "\\app.asar");

        if (!hasNexCord || expected !== latest) {
            showNotice(
                "Discord se actualizó. NexCord necesita repararse.",
                "Reparar ahora",
                async () => {
                    try {
                        const repairBat = "NexCord-Repair.bat";
                        if (fs.existsSync(repairBat)) {
                            child_process.exec(`start "" "${repairBat}"`, { shell: true });
                        }
                        relaunch();
                    } catch (err) {
                        console.error("[NexCord] Repair failed", err);
                    }
                }
            );
        }

        localStorage.setItem("NexCord_lastDiscordPath", latest);
    } catch (err) {
        console.error("[NexCord] checkDiscordVersionChange error", err);
    }
}

function getLatestDiscordApp() {
    const base = process.env.LOCALAPPDATA + "\\Discord";
    if (!fs.existsSync(base)) return null;

    const dirs = fs.readdirSync(base).filter(d => /^app-\d+/.test(d));
    if (dirs.length === 0) return null;

    dirs.sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
    return base + "\\" + dirs[dirs.length - 1];
}

async function init() {
    checkDiscordVersionChange();
    await onceReady;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    createAndAppendStyle("NexCord-titlebar-position", coreStyleRootNode).textContent = `
        body:not(.NexCord-stealth):not(.NexCord-compact) [class*="title_c38"] {
