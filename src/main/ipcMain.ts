...
ipcMain.handle(IpcEvents.WORLD_BOMB_TYPE, async (event, text: string, delay: number = 50) => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "NexCord-wb-"));
...
function runPowershellScript(psScript: string): Promise<void> {
    ...
    const { join } = require("path");
    const { tmpdir } = require("os");
    const tempDir = mkdtempSync(join(tmpdir(), "NexCord-ps-"));
...
ipcMain.handle(IpcEvents.WORLD_BOMB_SEQUENCE, async (
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "NexCord-wbs-"));
...
const OFFICIAL_UPDATE_URL = `https://git.${domain}/NexCord/NexCord/releases/download/latest/NexCord-Installer.exe`;

ipcMain.handle(IpcEvents.NexCord_DOWNLOAD_AND_RUN, async (event, url: string) => {
    if (!validateSender(event)) throw new Error("Unauthorized IPC invocation");
    ...
    const tmpPath = path.join(os.tmpdir(), "NexCordUpdate-Setup.exe");
    ...
    const { response } = await dialog.showMessageBox({
        ...
        defaultId: 0,
        title: "NexCord Update",
        message: "A NexCord update is available.",
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async (event) => {
    ...
    const tmpDir = path.join(os.tmpdir(), "NexCord-VBCable");
    ...
    try {
        ...
        console.error("[NexCord] VBCable install failed:", err);
