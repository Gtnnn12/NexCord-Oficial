...
function patchResourcesDir(resourcesDir: string) {
    try {
        ...
        // Check if already patched cleanly with app/index.js pointing to NexCord
        const loaderIndex = join(appDir, "index.js");
        if (existsSync(loaderIndex)) {
            try {
                const content = readFileSync(loaderIndex, "utf-8");
                if (content.includes("NexCord")) return; // Already patched
            } catch {}
        }
        ...
        console.info(`[NexCord] Auto-injecting into ${resourcesDir}...`);
        ...
        const indexJs = [
            ...
            "} catch (e) {",
            "    console.error(\"[NexCord] Injection failed, falling back to original _app.asar:\", e);",
...
function patchAllDiscordInstallations() {
    ...
    try {
        ...
        console.error("[NexCord] Error in patchAllDiscordInstallations:", err);
    }
}
