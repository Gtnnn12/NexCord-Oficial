 ...
 * NexCord-index.js reads this on next startup (before any file is locked)
 * and performs the actual file-swap then.
 */
export const PENDING_UPDATE_MARKER = join(app.getPath("temp"), "NexCord-pending-update.json");
const STAGING_DIR = join(app.getPath("temp"), "NexCord-pending-update");
...
async function fetchUpdates(): Promise<boolean> {
    ...
    const asset = (data.assets as any[])?.find(
        (a: any) => a.name.toLowerCase().endsWith(".exe")
    );
...
async function stageUpdate(): Promise<boolean> {
        ...
        try {
            ...
            if (marker.stagingDir && existsSync(marker.stagingDir)) {
                if (!pendingVersion || marker.version === pendingVersion) {
                    console.log(`[NexCordUpdater] Update ${marker.version} is already staged on disk. Skipping download.`);
    ...
    try {
        ...
        // Save zip to temp
        const zipPath = join(app.getPath("temp"), `NexCord-update-${Date.now()}.zip`);
        ...
        return await new Promise<boolean>((resolve, reject) => {
            exec(`powershell -NoProfile -NonInteractive -Command "${psExtract}"`, err => {
                ...
                // Write marker so NexCord-index.js knows to apply on next boot
