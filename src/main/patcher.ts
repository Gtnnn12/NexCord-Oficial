        ...
        constructor(options: BrowserWindowConstructorOptions) {
            ...
            if (process.platform === "win32" && winMaterial && winMaterial !== "none") {
                try {
                    ...
                    if (!applied) {
                        console.warn("[NexCord] No background material API available on this system");
                    }
                } catch (e) {
                    console.error("[NexCord] setBackgroundMaterial failed:", e);
                }
            }
...
function patchWebContents(wc: electron.WebContents) {
    if ((wc as any)._NexCordPatched) return;
    (wc as any)._NexCordPatched = true;
    ...
    wc.on("did-create-window", (childWin, details) => {
        ...
        if ((childWc as any)._NexCordPatched) return;
        (childWc as any)._NexCordPatched = true;
        ...
        (electron.ipcMain as any).handle = function(channel: string, listener: any) {
            ...
            try {
                ...
                if (e?.message?.includes?.("Attempted to register a second handler")) {
                    console.warn(`[NexCord] Ignored duplicate IPC handler for '${channel}'`);
                    return;
                }
                throw e;
            }
