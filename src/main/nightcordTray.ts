...
let NexCordTrayInstance: Tray | null = null;

/**
 * Initializes the NexCord system tray icon.
 ...
 */
export function initNexCordTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            NexCordTrayInstance = new Tray(icon);
            NexCordTrayInstance!.setToolTip("NexCord");
            ...
            const trayMenu = Menu.buildFromTemplate([
                ...
                { type: "separator" },
                {
                    label: "Uninstall NexCord",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "NexCord", "Uninstall NexCord.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "NexCord", "Uninstall NexCord.exe"),
                            join(process.env.PROGRAMFILES || "", "NexCord", "Uninstall NexCord.exe"),
                        ];
            ...
            NexCordTrayInstance!.setContextMenu(trayMenu);
            NexCordTrayInstance!.on("click", () => {
                const wins = electron.BrowserWindow.getAllWindows();
                const main = wins.find(w => !w.isDestroyed()) ?? wins[0];
                if (main) {
